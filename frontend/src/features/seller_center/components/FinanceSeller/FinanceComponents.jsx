...
export const StatCard = ({ title, value, icon, color, subText, loading }) => (
  <Card
    bordered={false}
    style={{ height: "100%", borderRadius: 12, boxShadow: "0 2px 10px rgba(0,0,0,0.03)" }}
    ...
    <Skeleton loading={loading} active paragraph={{ rows: 1 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          ...
          {subText && <div style={{ marginTop: 8, fontSize: 12, color: "#8c8c8c" }}>{subText}</div>}
        </div>
        <div
          style={{
  ...
  <div style={{ background: "#fff", padding: "24px 32px", borderBottom: "1px solid #f0f0f0" }}>
    <Row justify="space-between" align="middle">
      <Col>
        ...
        <Text type="secondary">Quản lý dòng tiền và hiệu quả kinh doanh của cửa hàng</Text>
      </Col>
      <Col>
        <Space>
...
export const RevenueChart = ({ data, loading }) => {
  const chartConfig = {
    ...
    areaStyle: () => ({ fill: `l(270) 0:#ffffff 0.5:${THEME.primary}10 1:${THEME.primary}30` }),
    legend: { position: "top" },
    yAxis: {
      label: { formatter: (v) => `${Number(v) / 1000}k` }
    },
    tooltip: {
      formatter: (datum) => ({ name: datum.metric, value: formatCurrency(datum.value) })
    }
  };

  return (
    <Card
      title={<Space><LineChartOutlined style={{ color: THEME.primary }} /> Biểu đồ tăng trưởng</Space>}
      bordered={false}
...
export const CashFlowForecast = ({ pendingBalance }) => (
  <Card
    title={<Space><CalendarOutlined style={{ color: THEME.primary }} /> Dự báo dòng tiền</Space>}
    bordered={false}
    style={{ borderRadius: 12, height: "100%" }}
    bodyStyle={{ padding: 0 }}
  >
      ...
      <div style={{ marginTop: 12 }}>
        {[7, 14, 30].map((days) => (
          ...
          >
            ...
            {/* Logic hiển thị mock dựa trên tổng pending */}
