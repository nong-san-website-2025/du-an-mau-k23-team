...
/* ==================================================================================
   CACHE HELPERS (Thêm mới để lưu/lấy dữ liệu local)
   ================================================================================== */
const CACHE_KEY = {
  STATS: "DB_CACHE_STATS",
  PENDING: "DB_CACHE_PENDING",
  PROCESSING: "DB_CACHE_PROCESSING",
  TOP_PROD: "DB_CACHE_TOP_PROD",
  REVIEWS: "DB_CACHE_REVIEWS",
};

const getCache = (key, initialValue) => {
  try {
...
export default function Dashboard() {
  // THAY ĐỔI 1: Initial state lấy từ Cache thay vì giá trị rỗng/mặc định
  // loading set mặc định là false để hiện UI ngay (dữ liệu cũ), API chạy ngầm sẽ update sau
  const [loading, setLoading] = useState(false);
  const [accessDenied, setAccessDenied] = useState(false);

  const [pendingOrders, setPendingOrders] = useState(() =>
    getCache(CACHE_KEY.PENDING, [])
  );
  const [processingOrders, setProcessingOrders] = useState(() =>
    getCache(CACHE_KEY.PROCESSING, [])
  );

  // Stats State
  const [stats, setStats] = useState(() =>
    getCache(CACHE_KEY.STATS, {
      today: 0,
      yesterday: 0,
      week: 0,
      prevWeek: 0,
  ...
  const [topProducts, setTopProducts] = useState(() =>
    getCache(CACHE_KEY.TOP_PROD, [])
  );
  const [reviewActivities, setReviewActivities] = useState(() =>
    getCache(CACHE_KEY.REVIEWS, [])
  );
  ...
  const loadData = async (opts = {}) => {
    try {
      // Chỉ set loading true nếu chưa có dữ liệu cached nào (lần đầu tiên vào app chưa lưu gì)
      // Nếu đã có data hiển thị rồi thì không cần loading quay quay làm phiền mắt
      // if (!opts.silent && pendingOrders.length === 0 && stats.today === 0)
      //   setLoading(true);
      ...
      // THAY ĐỔI 2: Lưu dữ liệu mới vào State và Cache
      const newPending = pend.data || [];
      const newProcessing = proc.data || [];

      setPendingOrders(newPending);
      setCache(CACHE_KEY.PENDING, newPending);

      setProcessingOrders(newProcessing);
      setCache(CACHE_KEY.PROCESSING, newProcessing);
  ...
  const loadReviewActivities = async () => {
    try {
      ...
      const activities = (res.data.results || []).map((a) => ({
        message: a.message,
        created_at: a.created_at,
        type: "review",
      }));

      // THAY ĐỔI 3: Lưu reviews vào cache
      setReviewActivities(activities);
      setCache(CACHE_KEY.REVIEWS, activities);
    } catch {}
  };
  ...
  const computeStats = (orders, validIds) => {
    ...
    // THAY ĐỔI 4: Lưu Stats vào cache
    setStats(newStats);
    setCache(CACHE_KEY.STATS, newStats);
    ...
    const topProdData = last30.products.slice(0, 5);

    // THAY ĐỔI 5: Lưu Top Products vào cache
    setTopProducts(topProdData); // Lấy top 5 cho đẹp
    setCache(CACHE_KEY.TOP_PROD, topProdData);
  };
  ...
  return (
    ...
    >
      ...
      >
        <Space align="center">
          ...
          <div>
            ...
            <Text type="secondary">
              Chào mừng trở lại, chúc bạn một ngày buôn bán đắt khách!
            </Text>
          </div>
        </Space>
      ...
      {/* STATS SECTION */}
      {/* Loading ở đây được truyền là loading state nhưng do ta init load=false 
          nên nó sẽ render cached data ngay lập tức */}
      <StatsSection items={statsItems} loading={false} />
                ...
                <Space align="start">
                  ...
                  <div>
                    ...
                    {pendingOrders.length > 0 && (
                      <Tag color="blue">Cần xử lý ngay</Tag>
                    )}
                  </div>
                </Space>
              </Card>
              ...
              <Card
                title={
                  <Space>
                    ...
                    <span>Top sản phẩm (30 ngày)</span>
                  </Space>
                }
                bordered={false}
