...
export default function PromotionsPage() {
  ...
  const fetchData = async (params = {}) => {
    setLoading(true);
    try {
      // PHẢI dùng getVouchers (trỏ tới /vouchers/), KHÔNG dùng getPromotionsOverview (trỏ tới /overview/)
      const res = await getVouchers(params);

      console.log("Dữ liệu nhận được:", res);

      if (res && res.results) {
        setData(res.results); // Nếu có phân trang (Pagination)
      } else if (Array.isArray(res)) {
        setData(res); // Nếu không có phân trang
      } else {
        setData([]);
      }
    } catch (err) {
