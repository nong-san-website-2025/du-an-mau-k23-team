...
export default function ProductDetailDrawer({ visible, product, onClose }) {
    ...
    <Space direction="vertical" style={{ width: "100%" }}>
      ...
      >
        ...
        <Descriptions.Item label="Cửa hàng">
          <Space>
            <ShopOutlined /> {product.seller_name}
          </Space>
        </Descriptions.Item>
  ...
  const renderBusinessInfo = () => {
    ...
    const percentOrdered =
      isSeason && product.estimated_quantity > 0
        ? Math.round(
            (product.ordered_quantity / product.estimated_quantity) * 100
          )
        : 0;
        ...
        >
          <Descriptions.Item label="Trạng thái kho">
            <Tag color={availabilityConfig[product.availability_status]?.color}>
              {availabilityConfig[product.availability_status]?.text ||
          ...
          <Descriptions.Item label="Hoa hồng sàn">
            {product.commission_rate ? (
              <Tag color="green">{product.commission_rate * 100}%</Tag>
            ) : (
                ...
                <Row gutter={[16, 8]}>
                  <Col span={12}>
                    ...
                    {product.season_start
                      ? dayjs(product.season_start).format("DD/MM/YYYY")
                      : "---"}
                  </Col>
                  <Col span={12}>
                ...
                <div>
                  ...
                  >
                    ...
                    <Text strong>
                      ...
                      {intcomma(product.estimated_quantity)} {product.unit}
                    </Text>
                  </div>
                  <Progress
                    percent={percentOrdered}
                    size="small"
                    status="active"
                    strokeColor="#722ed1"
                  />
    ...
    >
      {product.images?.map((img, idx) => (
        <div
          ...
          style={{
            position: "relative",
            border: img.is_primary ? "2px solid #1890ff" : "1px solid #ddd",
            borderRadius: 4,
            overflow: "hidden",
          }}
        >
  ...
  const items = [
    ...
    },
    {
      ...
      label: (
        <span>
          <FileImageOutlined /> Hình ảnh ({product.images?.length || 0})
        </span>
      ),
    ...
    >
      ...
      {isUpdate && (
        <Alert
          ...
          description={
            <div>
              Sản phẩm này vừa được Người bán chỉnh sửa lúc{" "}
              <b>{dayjs(product.updated_at).format("HH:mm DD/MM/YYYY")}</b>.
              <br />
      ...
      >
        <div
          style={{ border: "1px solid #f0f0f0", borderRadius: 6, padding: 2 }}
        >
          <Image
            width={80}
            height={80}
