...
import { getPublicVouchersForSeller, getMyVouchers, claimVoucher } from "../../admin/services/promotionServices";
...
const StoreDetail = () => {
    ...
    useEffect(() => {
        const fetchStoreData = async () => {
            ...
            try {
                ...
                const [storeRes, productsRes, publicVouchersRes, myVouchersRes] = await Promise.all([
                    axios.get(`http://localhost:8000/api/sellers/${id}/`, { headers: authHeader }),
                    axios.get(`http://localhost:8000/api/products/?seller=${id}&ordering=-created_at`),
                ...
                if (myVouchersRes) {
                    const claimedCodes = new Set(myVouchersRes.map(uv => uv.voucher.code));
    ...
    const handleCopyVoucher = (v) => { /* ... giữ nguyên code của bạn ... */ };
    ...
    const handleFollow = async () => { /* ... giữ nguyên code của bạn ... */ };
    const handleOpenChat = () => { /* ... giữ nguyên code của bạn ... */ };
    const ratingStats = useMemo(() => { /* ... giữ nguyên code của bạn ... */ }, [products]);
    ...
    if (loading) { return <div style={{ textAlign: "center", marginTop: 50 }}><Spinner animation="border" /></div>; }
    if (!store) { return <p className="text-center my-5">❌ Không tìm thấy cửa hàng.</p>; }
